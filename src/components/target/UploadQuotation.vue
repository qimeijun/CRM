  ...
  <div class="target-upload-report">
    ...
    <div class="upload-report_div">
      ...
      <div>
        ...
        >
          <div v-if="btnShow">
            <el-button size="small" type="primary">{{$t("target.quotation.uploadBtn[0]")}}</el-button>
            <span slot="tip" class="el-upload__tip">&nbsp;&nbsp;{{$t("target.quotation.textTip[1]")}}</span>
          </div>
          <div v-else>
            <p>
              <span>{{fileName}}&nbsp;&nbsp;&nbsp;&nbsp;</span>
              <el-button type="text">{{$t("target.quotation.uploadBtn[1]")}}</el-button>
            </p>
          </div>
        </el-upload>
    ...
    <div class="upload-report_button">
      <el-button :disabled="btnDisabled" size="small" type="primary" @click="submitFile()">{{$t("target.quotation.ok")}}</el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async updatebtnShow(file) {
      ...
      if (!isLt5M) {
        this.$message.error(this.$t("target.quotation.textTip[1]"));
    ...
    submitFile() {
      if (this.type == "update") {
        ...
        // update
        this.$http
          .post("/target/company/report/update", {
            ...
            if (res.iworkuCode == 200) {
              this.$imessage({
                content: this.$t("target.quotation.updateContent"),
        ...
        // add
        this.$http
          .post("/target/company/item/report/save", {
            companyId: this.targetid,
            researchFile: this.researchFile
          }).then(res => {
            if (res.iworkuCode == 200) {
              this.$imessage({
                content: this.$t("target.quotation.addContent"),
                type: "success" // 错误提示 error
              });
              this.$emit("close");
              // 重置数据
