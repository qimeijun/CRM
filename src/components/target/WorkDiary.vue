<template>
    <WorkDiary type="target" :id="$route.params.targetid" :itemid="$route.params.itemid" :isAllow="isAllow"></WorkDiary>
</template>
<script>
...
export default {
    ...
    async created() {
         ...
         * 2、否则不允许
         */
        let res = await this.$http.get(`/target/company/infobypk/${this.$route.params.targetid}`);
        if (res.iworkuCode == 200 && res.datas) {
            // 如果这个目标公司未被分配，那么就不能进行操作
            res.datas.targetCompany.ownUser == 1 ? this.isAllow = false : (function (_this, result){
                result.datas.targetCompany.ownerUserId = result.additionalParameters.targetCompanyUserInfo.id;
    ...
    methods: {
        distinguishUserRoles(targetCompany) {
            if (this.userInfo.userRole == this.$global.userRole.superAdministrator) {
                ...
                // 这个登录用户就是这个目标公司的持有者
                this.isAllow = true;
            }  else if (this.userInfo.userRole == this.$global.userRole.projectManager && this.userInfo.id == targetCompany.itemProjectManager) {
                ...
                this.$http.post("/user/info/find/user", {id: targetCompany.itemProjectManager}).then(ures => {
